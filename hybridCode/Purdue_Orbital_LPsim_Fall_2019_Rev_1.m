...
oxidizer = 'HNO3(L)';                                                  % Oxidizer choice (match name to CEA inp files!!!)

[t_c(i), o_f_cea(i), gamma(i), c_star(i), MW] = callCEA1(o_f(i), fuel, oxidizer);

a = .034;                                                                  %[m^(1+2n) kg^(-n) s^(n-1)] regression rate coefficient r=a*Gox^n (empirical data from research)
n = .96;                                                                   %regression rate coefficient r=a*Gox^n (empirical data from research)
